## 発動
#弾が埋まっていればreturn
execute anchored eyes positioned ^ ^-0.1 ^0.5 unless block ~ ~ ~ #werewolf:no_collision run return run particle minecraft:dust{color:[0.0,0.0,0.0],scale:1.8} ~ ~ ~ 0.3 0.3 0.3 0 5 force @a

#アイテム用意
execute anchored eyes run summon item ^ ^-0.1 ^0.5 {Item:{id:"minecraft:carrot_on_a_stick",components:{"minecraft:fire_resistant":{},"minecraft:custom_model_data":99999}},Health:1s,PickupDelay:10000,Tags:["magic_bullet","thunder","not_yet"],NoGravity:true,Age:5950}
#投擲者を記録
...
#ベクトル計算(max:1.6)
summon area_effect_cloud ^ ^ ^1.6 {Tags:["magic_bullet_motion"]}
#x(reserve_1)
