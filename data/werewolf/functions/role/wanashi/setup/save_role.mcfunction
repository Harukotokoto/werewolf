...
#return判定
execute unless entity @a[tag=team_wanashi,tag=!dummy_role] run return fail
...
'[{"interpret":true,"nbt":"role.wanashi.name","storage":"werewolf:","color":"#4e5c24"},{"text":"   "},{"selector":"@a[tag=team_wanashi,tag=!dummy_role]","color":"red","font":"default"}]'
