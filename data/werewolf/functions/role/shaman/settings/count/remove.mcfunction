#第三陣営は1役職に制限
execute if data storage werewolf: role.shaman{camp:"third"} if data storage werewolf: role.shaman{count:0} run return fail
execute if data storage werewolf: role.shaman{camp:"third"} run data modify storage macro: util.role_processing.third_camp set value 0
