#第三陣営の場合は1役職に制限
execute if data storage werewolf: role.kyouyuu{camp:"third"} unless data storage macro: util.role_processing{third_camp:0} run return fail
execute if data storage werewolf: role.kyouyuu{camp:"third"} run data modify storage macro: util.role_processing.third_camp set value 1
