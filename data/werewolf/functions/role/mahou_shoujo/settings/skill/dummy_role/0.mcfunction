...
unless data storage werewolf: role.$(result){camp:"wolf"} \
unless data storage werewolf: role.$(result){camp:"maniac"} \
unless data storage werewolf: role.$(result){camp:"third"} \
unless data storage werewolf: role.$(result){display_dummy_settings:false} run \
function werewolf:role/mahou_shoujo/settings/skill/dummy_role/1 with storage macro: util.role_processing
