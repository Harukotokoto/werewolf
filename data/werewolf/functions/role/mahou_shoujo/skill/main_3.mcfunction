## 発動
#アイテム用意
execute anchored eyes run summon item ^ ^-0.1 ^0.5 {Item:{id:"minecraft:carrot_on_a_stick",components:{"minecraft:fire_resistant":{},"minecraft:custom_model_data":99999}},Health:1s,PickupDelay:10000,Tags:["magic_bullet","ms","not_yet"],NoGravity:true,Age:5950}

#弾が埋まっていればダメージ処理後return
execute anchored eyes positioned ^ ^-0.1 ^0.5 unless block ~ ~ ~ #werewolf:no_collision run return run function werewolf:role/mahou_shoujo/skill/damage
