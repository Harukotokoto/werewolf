## 発動(ダミー)
#アイテム用意
execute anchored eyes run summon item ^ ^-0.1 ^0.5 {Item:{id:"minecraft:carrot_on_a_stick",components:{"minecraft:fire_resistant":{},"minecraft:custom_model_data":99999}},Health:1s,PickupDelay:10000,Tags:["magic_bullet","ms_dummy","not_yet"],NoGravity:true,Age:5950}

#弾が埋まっていればダメージ処理後return
execute anchored eyes positioned ^ ^-0.1 ^0.5 unless block ~ ~ ~ #werewolf:no_collision run return run function werewolf:role/mahou_shoujo/skill/damage_dummy

#投擲者を記録
data modify entity @e[type=item,tag=magic_bullet,tag=not_yet,sort=nearest,limit=1] Thrower set from entity @s UUID
#投擲者の向きを代入
data modify entity @e[type=item,tag=magic_bullet,tag=not_yet,sort=nearest,limit=1] Rotation set from entity @s Rotation
#位置反映のためのダメージ
damage @e[type=item,limit=1,sort=nearest,tag=magic_bullet,tag=not_yet] 0.0

#ベクトル計算(max:1.6)
summon area_effect_cloud ^ ^ ^1.5 {Tags:["magic_bullet_motion"]}
#x(reserve_1)
