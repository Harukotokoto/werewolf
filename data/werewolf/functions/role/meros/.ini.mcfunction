...
$data modify storage werewolf: role.$(result).camp set value "villager"
# 最大人数を設定
$data modify storage werewolf: role.$(result).count_max set value 999
# ダミーの可否
$execute unless data storage werewolf: role.$(result).dummy run data modify storage werewolf: role.$(result).dummy set value true
# 設定等の表示可否
$execute unless data storage werewolf: role.$(result).display_settings run data modify storage werewolf: role.$(result).display_settings set value true
# ダミー役職設定の表示可否
$data modify storage werewolf: role.$(result).display_dummy_settings set value true
...
# 回数制限
#$execute unless data storage werewolf: role.$(result).skill_0.limit run data modify storage werewolf: role.$(result).skill_0.limit set value 1
#スキル1(日数回復)
# クールタイム
...
##その他
# 死亡するまでの時間(秒)
$execute unless data storage werewolf: role.$(result).death_time run data modify storage werewolf: role.$(result).death_time set value 3
# 報酬獲得までの時間(分)
$execute unless data storage werewolf: role.$(result).reward_time run data modify storage werewolf: role.$(result).reward_time set value 12
# 移動検知のためのスコアボード作成
#スニークした距離
scoreboard objectives add hm_crouch_one_cm minecraft.custom:minecraft.crouch_one_cm
#ダッシュした距離
scoreboard objectives add hm_sprint_one_cm minecraft.custom:minecraft.sprint_one_cm
#歩いた距離
scoreboard objectives add hm_walk_one_cm minecraft.custom:minecraft.walk_one_cm
#水上を歩いた距離
scoreboard objectives add hm_walk_on_water_one_cm minecraft.custom:minecraft.walk_on_water_one_cm
#水中を歩いた距離
scoreboard objectives add hm_walk_under_water_one_cm minecraft.custom:minecraft.walk_under_water_one_cm
#泳いだ距離
scoreboard objectives add hm_swim_one_cm minecraft.custom:minecraft.swim_one_cm
#上下移動した距離
scoreboard objectives add hm_y_one_cm dummy
#空中を移動した距離
scoreboard objectives add hm_fly_one_cm minecraft.custom:minecraft.fly_one_cm
#落下した距離
scoreboard objectives add hm_fall_one_cm minecraft.custom:minecraft.fall_one_cm
# 死亡判定のスコアボード作成
scoreboard objectives add hm_death dummy
# 報酬配布のためのスコアボード作成
scoreboard objectives add hm_reward dummy
