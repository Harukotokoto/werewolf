...
#return判定
execute unless entity @a[tag=team_kyoushin,tag=!dummy_role] run return fail
...
'[{"interpret":true,"nbt":"role.kyoushin.name","storage":"werewolf:","color":"#4e5c24"},{"text":"   "},{"selector":"@a[tag=team_kyoushin,tag=!dummy_role]","color":"red","font":"default"}]'
