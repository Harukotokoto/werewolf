...
#return判定
execute unless entity @a[tag=team_reinou,tag=!dummy_role] run return fail
...
'[{"interpret":true,"nbt":"role.reinou.name","storage":"werewolf:","color":"#4e5c24"},{"text":"   "},{"selector":"@a[tag=team_reinou,tag=!dummy_role]","color":"green","font":"default"}]'
