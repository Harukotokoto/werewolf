...
#アイテム用意
execute anchored eyes run summon item ^ ^0.25 ^ {Item:{id:"minecraft:carrot_on_a_stick",components:{"minecraft:fire_resistant":{},"minecraft:custom_model_data":99999}},Health:1s,PickupDelay:10000,Tags:["cupid_arrow","not_yet"],NoGravity:false}
#投擲者を記録
data modify entity @e[type=item,tag=cupid_arrow,tag=not_yet,sort=nearest,limit=1] Thrower set from entity @s UUID
#位置反映のためのダメージ
...
#Motion代入
execute store result entity @e[type=item,tag=cupid_arrow,tag=not_yet,limit=1,sort=nearest] Motion[0] double 0.01 run scoreboard players get #GameManager reserve_1
execute store result entity @e[type=item,tag=cupid_arrow,tag=not_yet,limit=1,sort=nearest] Motion[1] double 0.01 run scoreboard players get #GameManager reserve_2
execute store result entity @e[type=item,tag=cupid_arrow,tag=not_yet,limit=1,sort=nearest] Motion[2] double 0.01 run scoreboard players get #GameManager reserve_3
