...
##タスクの抽選漏れ
execute at @s[tag=!active_task] align xyz run summon block_display ~ ~ ~ {block_state:{Name:"minecraft:brown_mushroom"},Tags:["dummy_task"]}

##タスク場所
#タスク場所にタスクを設置
execute as @s[tag=active_task] at @s unless block ~ ~ ~ minecraft:brown_mushroom run setblock ~ ~ ~ brown_mushroom
execute as @s[tag=active_task] at @s align xyz run summon interaction ~0.5 ~ ~0.5 {Tags:["1click_task","brown_mushroom"],width:1.1f,height:1.1f}
#タスクが見やすいようディスプレイ表示
