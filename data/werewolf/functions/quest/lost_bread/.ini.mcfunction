...
execute if data storage werewolf: settings.shop.price{bread:0} if data storage werewolf: settings.shop.price{breads:0} run return run function werewolf:.util/quest_processing/start/2
#今回のみ実行不可ならクエストを後ろにして再抽選
execute store result score #GameManager reserve_0 run data get storage werewolf: gamestate.day_count.number
execute if score #GameManager reserve_0 matches ..2 run data remove storage werewolf: quest_list_shuffle[0]
execute if score #GameManager reserve_0 matches ..2 run data modify storage werewolf: quest_list_shuffle append value "lost_bread"
execute if score #GameManager reserve_0 matches ..2 run return run function werewolf:.util/quest_processing/start/0

## クエスト制限時間(実行不可ならreturn)
#ここに制限時間
scoreboard players set #GameManager reserve_0 60
...
#内容
data modify storage werewolf: gamestate.quest.text append value '{"text":"村長のおふれ"}'
data modify storage werewolf: gamestate.quest.text append value '{"text":"目標:  -"}'
...
tellraw @a {"text":"[システム] クエスト発生: 村長のおふれ"}
tellraw @a {"text":"[システム] 食料が底を尽きかけている… ジャガイモを食べましょう。"}
...
## クエスト実行
function werewolf:quest/lost_bread/