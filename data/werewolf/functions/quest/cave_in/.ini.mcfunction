...
execute if data storage werewolf: settings{field:"village"} run return run function werewolf:.util/quest_processing/start/2
##今回のみ実行不可ならreturn
...
#内容
data modify storage werewolf: gamestate.quest.text append value '{"text":"落盤"}'
...
tellraw @a {"text":"[システム] クエスト発生: 落盤"}
tellraw @a {"text":"[システム] どこかで嫌な音がする…"}
...
## クエスト実行
function werewolf:quest/cave_in/