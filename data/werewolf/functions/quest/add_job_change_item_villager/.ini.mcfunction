...
#execute unless data storage werewolf: {active_role_list:["mura"]} run return run function werewolf:.util/quest_processing/start/2
execute unless entity @a[tag=team_mura] run return run function werewolf:.util/quest_processing/start/2
##今回のみ実行不可ならreturn
...
#内容
data modify storage werewolf: gamestate.quest.text append value '{"text":"村人の仕事探し"}'
data modify storage werewolf: gamestate.quest.text append value '{"text":"目標:  -"}'
data modify storage werewolf: gamestate.quest.text append value '{"text":"報酬:  就職案内書(村人専用)のショップ取り扱い"}'
...
tellraw @a {"text":"[システム] クエスト発生: 村人の仕事探し"}
tellraw @a {"text":"[システム] ショップに就職案内書が置かれたようだ"}
...
## クエスト実行
function werewolf:quest/add_job_change_item_villager/