...
execute as @a run function werewolf:role/$(result)/setup/reset_0
$execute unless data storage macro: util.role_processing{count:0} run \
execute as @a run function werewolf:role/$(result)/setup/reset_1
...
# 再帰処理
function werewolf:.util/role_processing/reset/everyone/0 with storage macro: util.role_processing